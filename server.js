...
app.post('/api/inventory', requireAuth, async (request, response, next) => { try { response.status(201).json(await store.saveInventory(request.body)); } catch (error) { next(error); } });
app.patch('/api/inventory/:id/adjust', requireAuth, async (request, response, next) => { try { response.json(await store.adjustInventory(request.params.id, request.body)); } catch (error) { next(error); } });
...
app.use((error, _request, response, _next) => {
  console.error(error);
  const known = /obligatori|insuficiente|negativ|no encontr|repetida/i.test(error.message);
